    ...
    <section id="how-it-works" className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="relative max-w-4xl mx-auto">
          {/* Connecting line */}
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-primary/40 via-primary/20 to-transparent hidden md:block -translate-x-1/2"></div>
          ...
          <div className="space-y-12">
            {steps.map((step, i) => {
              ...
              const isLeft = i % 2 === 0;
              return (
                <StepItem key={step.step} step={step} Icon={Icon} index={i} isLeft={isLeft} />
              );
            })}
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className={`flex-1 ${isLeft ? "md:text-right" : "md:text-left"}`}>
        <div className={`inline-block p-6 rounded-3xl border border-white/10 bg-card/80 backdrop-blur-sm hover:border-primary/30 transition-all duration-300 group`}>
          <span className="text-primary/50 text-sm font-bold font-mono block mb-2">
            {step.step}
          </span>
      ...
      <div className="relative hidden md:flex flex-shrink-0">
        <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-primary to-blue-600 flex items-center justify-center shadow-lg shadow-primary/30 z-10">
